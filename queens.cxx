...
void queens(unsigned int nQueens)
{
    ...
    while (!success && !choices.empty( ))
    {
        ...
        if (is_conflict(choices))
        {
            ...
            unsigned int top_val = choices.top();
            //check what the first val is:
            Stack<unsigned int> store;
            for(unsigned long i = choices.size(); i>1; --i){
                store.push(choices.pop());
            }
            unsigned int first_val = choices.top();
            for(unsigned long i = store.size(); i>0; --i){
                choices.push(store.pop());
            }
            if(top_val == nQueens && (first_val) == nQueens){
                //if top and first value are in the last column
                break; //no solution, no where else to go
            }
            else if((choices.size() == nQueens) && (top_val == nQueens)){
                //if at final row and column = nQueens, pop until empty
                for(unsigned long i = choices.size(); i>1; --i){
                    choices.pop();
                }
                first_val = choices.top();
                choices.pop();
                choices.push(first_val + 1);
                ++first_val;
                //increment first row by 1  column
            }
            else if(top_val == nQueens){
                //if in last column, but board is not filled
                choices.pop();
                unsigned int temp2 = choices.top();
                choices.pop();
                if(temp2 < nQueens)
                    choices.push(temp2 + 1);
            }
            else{
                choices.pop();
                choices.push(top_val + 1);
            }
        }
